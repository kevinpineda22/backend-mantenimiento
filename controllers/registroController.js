...
export const registrarFoto = async (req, res) => {
  ...
  try {
    const subirImagen = async (file, carpeta) => {
      ...
      const nombreLimpio = file.originalname.replace(/\s/g, "_").replace(/\.[^/.]+$/, '.webp');
...
export const registrarActividad = async (req, res) => {
  try {
    ...
    const { sede, actividad, fechaInicio, fechaFinal, precio, observacion, estado } = req.body;

    // Validar campos obligatorios
    if (!sede || !actividad || !fechaInicio || !fechaFinal || !precio || !estado) {
