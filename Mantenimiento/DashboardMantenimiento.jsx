...
const DashboardMantenimiento = () => {
    ...
    const getUserEmail = async () => {
      try {
        ...
        if (session?.user?.email) {
          ...
          if (esAdmin) {
            console.log(
              `🔑 Dashboard - Usuario ${email} identificado como SUPERADMIN`
            );
          }
        }
      } catch (error) {
  ...
  const calcularKPIs = useCallback((registros) => {
    ...
    for (const r of registros) {
      const estadoKey = normalizeEstado(r.estado);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (isSuperAdmin) {
        ...
        console.log(
          `🔓 Dashboard - Mostrando TODOS los registros (${resData.length}) para superadmin ${userEmail}`
        );
        ...
        console.log(
          `🔒 Dashboard - Mostrando ${filteredData.length} de ${resData.length} registros filtrados para ${userEmail}`
        );
      }
  ...
  const {
    ...
    const sedeStateMap = new Map(); // ✅ NUEVO: Mapa para Sede vs Estado

    for (const r of data) {
      const date = r?.fecha_inicio ? new Date(r.fecha_inicio) : null;
      // if (!date || Number.isNaN(date.getTime())) continue; // ⚠️ OJO: Si filtramos aquí, perdemos registros sin fecha para el gráfico de sedes.
      // Para mantener consistencia con los otros gráficos que dependen de fecha, lo dejaremos así, pero idealmente el gráfico de sedes podría mostrar todo.
      // Asumiremos que queremos mostrar lo mismo que en los otros gráficos temporales.
      if (!date || Number.isNaN(date.getTime())) continue;
      ...
      // ✅ Lógica Sede vs Estado
      const sedeKey = r.sede || "Sin Sede";
      if (!sedeStateMap.has(sedeKey)) sedeStateMap.set(sedeKey, {});
      const sedeEntry = sedeStateMap.get(sedeKey);
      sedeEntry[estadoKey] = (sedeEntry[estadoKey] || 0) + 1;
    }

    // ... (Lógica existente para stackedBarData) ...
    ...
    // ✅ NUEVO: Generar datos para Sede vs Estado
    const sortedSedesLabels = Array.from(sedeStateMap.keys()).sort();
    // Obtener todos los estados presentes en el mapa de sedes
    const allEstadosSede = Array.from(
      new Set(
        Array.from(sedeStateMap.values()).flatMap((entry) => Object.keys(entry))
      )
    );

    const sedeStateData = {
      labels: sortedSedesLabels,
      datasets: allEstadosSede.map((estado) => ({
        label: formatEstadoLabel(estado),
        data: sortedSedesLabels.map(
          (sede) => sedeStateMap.get(sede)?.[estado] || 0
        ),
        backgroundColor: getStatusColor(estado),
        borderColor: getStatusColor(estado),
        borderWidth: 1,
  ...
  // ✅ NUEVO: Opciones para Sede vs Estado
  const stackedSedeOptions = useMemo(
    () => ({
      ...defaultChartOptions,
      scales: {
        x: { stacked: true, title: { display: true, text: "Sede" } },
        y: {
          stacked: true,
          title: { display: true, text: "Cantidad de Registros" },
        },
    ...
    <div className="dsh-mnt-container">
      ...
      {data.length > 0 && (
        <div className="dsh-mnt-charts-grid">
          ...
          {/* ✅ NUEVO Gráfico: Registros por Sede y Estado (Stacked Bar) */}
          <div className="dsh-mnt-chart-card dsh-mnt-chart-card-full">
            <h3 className="dsh-mnt-chart-title">
              Distribución de Registros por Sede y Estado
            </h3>
            <div className="dsh-mnt-chart-container">
              {sedeStateData.labels.length > 0 ? (
                <Bar data={sedeStateData} options={stackedSedeOptions} />
              ) : (
                <div className="dsh-mnt-chart-empty">
                  Sin datos para mostrar
                </div>
