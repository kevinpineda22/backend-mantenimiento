...
const TareasRecibidas = () => {
  ...
  const getAssignmentType = (row) => {
    ...
    if (row.responsables_grupo && row.responsables_grupo.includes(userEmail))
      return {
        label: "👥 Grupal",
        value: "grupal",
        color: "#E8F5E9",
          ...
          return (
            <span
              style={{
                ...
                fontSize: "0.75rem",
                fontWeight: "bold",
                whiteSpace: "normal", // ⭐ CAMBIO: Permitir salto de línea
                textAlign: "center",
                display: "inline-block",
                lineHeight: "1.2",
                width: "100%", // ⭐ CAMBIO: Ocupar ancho disponible
              }}
            >
              {type.label}
            </span>
  ...
  const filterConfig = useMemo(() => {
    ...
    return {
      ...
      tipoPlaceholder: "Todos los tipos",
      tipoOptions: [
        { value: "directa", label: "👤 Directa" },
        { value: "grupal", label: "👥 Grupal" },
        { value: "supervision", label: "🏢 Supervisión Sede" },
      ],
      showDateRange: false,
      showSingleDate: true,
  ...
  const filteredData = useMemo(() => {
    ...
    if (filters.tipo) {
      filtered = filtered.filter((item) => {
        ...
        return type.value === filters.tipo;
      });
    }
  ...
  return (
    <div className="maint-form-container">
      ...
      {userEmail && (
        ...
        >
          <span>
            📨 Mostrando tareas para: <strong>{userEmail}</strong>
          </span>
