...
const HistorialActividadesPage = () => {
  ...
  const fetchHistorial = async () => {
    ...
    try {
      ...
      if (isSuperAdmin) {
        console.log(`🔓 Mostrando TODOS los registros (${data.length}) para superadmin ${userEmail}`);
        filteredHistorial = data;
      } else {
        filteredHistorial = data.filter(task => {
          return task.creador_email === userEmail || task.responsable === userEmail;
        });
        console.log(`🔒 Mostrando ${filteredHistorial.length} registros filtrados para ${userEmail}`);
      }
  ...
  const handleEditSubmit = async (e) => {
    ...
    const shouldTriggerCompletion = newFotoDespuesUploaded && editData.estado !== 'completado' && editData.estado !== 'no_completado';
    ...
    if (!editData.sede || !editData.actividad || !editData.fechaInicio || !editData.estado || !editData.responsable) {
      Swal.fire({ 
        icon: "error", 
    ...
    if (editData.fotoDespues instanceof File) formPayload.append("fotoDespues", editData.fotoDespues);

    try {
      const response = await fetch(`${apiBaseUrl}/actividades/full/${editData.id}`, { 
        method: "PUT", 
        body: formPayload 
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Error al actualizar la actividad");

      Swal.fire({
        icon: "success",
        title: "¡Éxito!",
        text: editData.notificarFinalizacion === 'true' 
  ...
  const handleSeguimientoSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/actividades/full/${selectedRowForSeguimiento.id}`, { 
        method: "PUT", 
        body: formPayload 
      });
  ...
  const handleDeleteImage = (row, tipo) => {
    Swal.fire({
      ...
      text: `¿Seguro que quieres eliminar la imagen de "${tipo === "antes" ? "Foto Antes" : "Foto Después"}"?`,
      icon: "warning",
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        setLoading(true);
        try {
          const response = await fetch(`${apiBaseUrl}/actividades/full/${row.id}/eliminar-imagen`, {
            method: "POST",
  ...
  const handleDeleteClick = (row) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setLoading(true);
        try {
          const response = await fetch(`${apiBaseUrl}/actividades/full/${row.id}`, { method: "DELETE" });
          const data = await response.json();
          if (!response.ok) throw new Error(data.error || "Error al eliminar el registro");
  ...
  const filterConfig = useMemo(() => ({
    ...
    showSearch: true,
    showCategory: true,
    categoryLabel: "Filtrar por Sede",
    categoryPlaceholder: "Todas las sedes",
    categories: [
      { value: "", label: "📍 Todas las sedes" },
      { value: "Plaza", label: "🏢 Plaza" },
      { value: "Villa Hermosa", label: "🏘️ Villa Hermosa" },
      { value: "Girardota Parque", label: "🌳 Girardota Parque" },
      { value: "Llano", label: "🌾 Llano" },
      { value: "Vegas", label: "🎰 Vegas" },
      { value: "Barbosa", label: "🏔️ Barbosa" },
  ...
  useEffect(() => {
    if (userEmail && filteredData.length > 0) {
      ...
      console.log("✅ Tareas donde ERES responsable (verás botón 🔄):", tareasComoResponsable.length);
      if (tareasComoResponsable.length > 0) {
        console.log("📋 Tareas con botón de redirigir:", 
  ...
  const columnas = useMemo(
    () => [
          ...
          row.foto_antes_url ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "4px" }}>
              ...
              <button 
                ...
                style={{ fontSize: "0.7rem", color: "#d33", background: "none", border: "none", cursor: "pointer", textDecoration: "underline" }} 
                title="Eliminar imagen"
              >
                Eliminar
              </button>
      ...
      },
      {
        name: "Acciones",
        cell: (row) => {
          const esResponsable = row.responsable?.toLowerCase() === userEmail?.toLowerCase();
    ...
    <div className="maint-form-container">
      ...
      {isSuperAdmin && (
        <div style={{
          ...
          <p style={{ margin: '5px 0 0 0', fontSize: '14px', color: '#856404' }}>
            Visualizando <strong>TODOS</strong> los registros del sistema ({historial.length} tareas)
          </p>
        </div>
      )}
          ...
          <form onSubmit={handleEditSubmit} className="maint-form-card">
            <div className="maint-historial__edit-grid">
              ...
              <div className="maint-historial__column">
                ...
                <div className="maint-form-group">
                  <label htmlFor="responsable" className="maint-form-label">Responsable (Correo)</label>
                ...
                <div className="maint-form-group">
                  <label htmlFor="nombre_solicitante" className="maint-form-label">👤 Nombre del Solicitante</label>
                  <input 
                    type="text" 
                    ...
                    style={{
                      borderColor: editData.nombre_solicitante ? '#2196F3' : undefined,
                      backgroundColor: editData.nombre_solicitante ? '#F0F8FF' : undefined
                    }}
                  />
                  <small style={{ display: 'block', marginTop: '4px', fontSize: '0.8em', color: '#666', fontStyle: 'italic' }}>
                    💡 Nombre de quien solicitó esta tarea (útil para tareas asignadas a Líderes)
                  </small>
                </div>
                <div className="maint-form-group">
              ...
              <div className="maint-historial__photo-section">
                ...
                <div className="maint-form-group maint-historial__photo-preview">
                  ...
                  <input type="file" name="fotoDespues" accept="image/*,.pdf" onChange={handleEditChange} className="maint-form-input" disabled={isOptimizingImage} />
                  {newFotoDespuesUploaded && <p style={{ color: '#4caf50', fontSize: '0.9rem', fontStyle: 'italic' }}>Nueva foto cargada.</p>}
                  <div className="maint-historial__image-wrapper">
      ...
      >
        ...
        {selectedRowForSeguimiento && (
          <div style={{ marginBottom: "20px", padding: "15px", backgroundColor: "#F8FAFC", borderRadius: "8px", borderLeft: "4px solid var(--maint-color-primary-medium)" }}>
            <h4 style={{ margin: "0 0 8px 0", fontSize: "1em" }}>
              📍 <strong>Sede:</strong> {selectedRowForSeguimiento.sede} | <strong>Actividad:</strong> {selectedRowForSeguimiento.actividad.substring(0, 100)}{selectedRowForSeguimiento.actividad.length > 100 ? '...' : ''}
            </h4>
            <p style={{ margin: "0", fontSize: "0.9em", color: "#6B7280" }}>
              Responsable: <strong>{selectedRowForSeguimiento.responsable}</strong>
                ...
                .map((entry, index) => (
                  ...
                  >
                    <div className="maint-seguimiento__header">
                      <span className="maint-seguimiento__autor">👤 {entry.autor}</span>
                      <span className="maint-seguimiento__fecha">🕒 {entry.fecha}</span>
                    </div>
        ...
        {userEmail && (
          <div style={{ marginBottom: "15px", padding: "10px", backgroundColor: "#E0F2FE", borderRadius: "6px", borderLeft: "4px solid #0891B2" }}>
        ...
        <div className="maint-form-group maint-historial__full-width">
          <label className="maint-form-label">✍️ <strong>Agregar Nuevo Seguimiento:</strong></label>
          <textarea 
          ...
          <div className="maint-historial__seguimiento-hint">
            💡 Este seguimiento será agregado al historial con tu email ({userEmail}) y la fecha y hora actuales.
          </div>
        </div>
